...
async function main() {
    ...
    console.log("Déclarez une phrase ou un mot et je vous dis si c'est un palindrome")

    do {
        if (essai > 0) {
            console.log("N'oubliez pas le point !!");
        }
        phrase = await sc.question("Veuillez saisir une phrase ou un mot qui se termine par un point : ");
        essai++;
    }
